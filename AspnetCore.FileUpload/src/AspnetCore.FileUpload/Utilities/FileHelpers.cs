...
namespace AspnetCore.FileUpload.Utilities
{
    public class FileHelpers
    {
        public static async Task<string> ProcessFormFile(IFormFile formFile,ModelStateDictionary modelState)
        {
            ...
            // a display name.
            MemberInfo property =typeof(Models.FileUpload).GetProperty(formFile.Name.Substring(formFile.Name.IndexOf(".", StringComparison.Ordinal) + 1));
            ...
            else
            {
                try
                {
                    ...
                    using (
                        var reader =new StreamReader(formFile.OpenReadStream(),new UTF8Encoding(false,true),true))
                    {
